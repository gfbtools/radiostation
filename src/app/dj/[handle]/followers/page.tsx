...
export default async function FollowersPage({ params }: FollowersPageProps) {
  ...
  const { data: followers } = await supabase
    .from('follows' as any)
    .select('follower_id, profiles:follower_id(id, handle, display_name, avatar_url, bio, city, country)')
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
      ...
      <UserList users={followers?.map((f: any) => f.profiles) || []} currentUserId={user?.id} followingSet={followingSet} />
    </div>
  )
}

function UserList({ users, currentUserId, followingSet }: { users: any[], currentUserId?: string, followingSet: Set<string> }) {
  if (users.length === 0) {
    return <div className="text-center py-16 text-gray-400">No followers yet.</div>
  }
